...
class BHProducer(Module):
  ...
  def beginFile(self, inputFile, outputFile, inputTree, wrappedOutputTree):
    ...
    self.out.branch("bh_l2_id", "I")
    self.out.branch("bh_l2_pdgid", "I")
    self.out.branch("bh_l2_pt", "F")
    self.out.branch("bh_l2_eta", "F")
    self.out.branch("bh_l2_phi", "F")
    self.out.branch("bh_l2_mass", "F")
    ...
    self.out.branch("bh_dr_l2j1", "F")
    self.out.branch("bh_dr_l2j2", "F")
    self.out.branch("bh_dr_l2j3", "F")
    self.out.branch("bh_dr_l2j4", "F")
    ...
    self.out.branch("cgbH_nl","B")
    self.out.branch("cgbH_region","I")
    self.out.branch("cgbH_l1_id","I")
    self.out.branch("cgbH_l1_pdgid","I")
    self.out.branch("cgbH_l1_pt","F")
    self.out.branch("cgbH_l1_eta","F")
    self.out.branch("cgbH_l1_phi","F")
    self.out.branch("cgbH_l1_mass","F")
    self.out.branch("cgbH_met", "F")
    self.out.branch("cgbH_met_phi", "F")
    self.out.branch("cgbH_dr_l1j1", "F")
    self.out.branch("cgbH_dr_l1j2", "F")
    self.out.branch("cgbH_dr_l1j3", "F")
    self.out.branch("cgbH_dr_l1j4", "F")
    self.out.branch("cgbH_mlj1", "F")
    self.out.branch("cgbH_mlj2", "F")
    self.out.branch("cgbH_mlj3", "F")
    self.out.branch("cgbH_mlj4", "F")
    self.out.branch("cgbH_mlj1j2", "F")
    self.out.branch("cgbH_mlj1j3", "F")
    self.out.branch("cgbH_mlj1j4", "F")
    self.out.branch("cgbH_mlj2j3", "F")
    self.out.branch("cgbH_mlj2j4", "F")
    self.out.branch("cgbH_mlj3j4", "F")
...
#      muon_pt=26
    ...
    # cgbH region
    ###################
    #cgbH region require one lepton and 3 jets
    #cgbH region lepton number selections
    cgbH_nl=False
    #cgbH region jet and bjet selection
    cgbH_jets=False
    # 0 ele and 1 muon or 0 muon and 1 ele
    cgbH_region=0
    cgbH_l1_id=-1
    cgbH_l1_pdgid=-99
    cgbH_l1_pt=-99
    ...
    if len(tightLeptons)==1 and tightLeptons[0].Pt()>15 and len(looseLeptons)==0:
      if (n_tight_muon==1)  or (n_tight_ele==1 ):
        cgbH_nl=True

    if cgbH_nl and n_tight_jet_in24>2:
      cgbH_jets=True

    if cgbH_nl:
      if self.is_mc:
        cgbH_met=event.MET_T1Smear_pt
        cgbH_met_phi=event.MET_T1Smear_phi
      else:
        cgbH_met=event.MET_T1_pt
        cgbH_met_phi=event.MET_T1_phi
      
      if len(tightMuons)==1 and abs(tightMuons_pdgid[0])==13:# and len(tightElectrons)==0 and abs(tightMuons_pdgid[0])==13:  
        cgbH_region=1
        cgbH_l1_id=tightMuons_id[0]
        cgbH_l1_pdgid=tightMuons_pdgid[0]
        cgbH_l1_pt=tightMuons[0].Pt()
        cgbH_l1_eta=tightMuons[0].Eta()
        cgbH_l1_phi=tightMuons[0].Phi()
        cgbH_l1_mass=tightMuons[0].M()
      
      if len(tightMuons)==0 and abs(tightElectrons_pdgid[0])==11: # and len(tightElectrons)==1 and abs(tightElectrons_pdgid[0])==11:  
        cgbH_region=2
        cgbH_l1_id=tightElectrons_id[0]
        cgbH_l1_pdgid=tightElectrons_pdgid[0]
        cgbH_l1_pt=tightElectrons[0].Pt()
        cgbH_l1_eta=tightElectrons[0].Eta()
        cgbH_l1_phi=tightElectrons[0].Phi()
        cgbH_l1_mass=tightElectrons[0].M()
    
    if cgbH_region>0:
      l1_v4_temp=TLorentzVector()
      l1_v4_temp.SetPtEtaPhiM(cgbH_l1_pt,cgbH_l1_eta,cgbH_l1_phi,cgbH_l1_mass)
      ...
      if n_tight_jet_in24>3:
        ...
        cgbH_dr_l1j1=l1_v4_temp.DeltaR(j1_v4_temp)
        cgbH_dr_l1j2=l1_v4_temp.DeltaR(j2_v4_temp)
        cgbH_dr_l1j3=l1_v4_temp.DeltaR(j3_v4_temp)
        cgbH_dr_l1j4=l1_v4_temp.DeltaR(j4_v4_temp)
        cgbH_mlj1=(l1_v4_temp+j1_v4_temp).M()
        cgbH_mlj2=(l1_v4_temp+j2_v4_temp).M()
        cgbH_mlj3=(l1_v4_temp+j3_v4_temp).M()
        cgbH_mlj4=(l1_v4_temp+j4_v4_temp).M()
        cgbH_mlj1j2=(l1_v4_temp+j1_v4_temp+j2_v4_temp).M()
        cgbH_mlj1j3=(l1_v4_temp+j1_v4_temp+j3_v4_temp).M()
        cgbH_mlj1j4=(l1_v4_temp+j1_v4_temp+j4_v4_temp).M()
        cgbH_mlj2j3=(l1_v4_temp+j2_v4_temp+j3_v4_temp).M()
        cgbH_mlj2j4=(l1_v4_temp+j2_v4_temp+j4_v4_temp).M()
        cgbH_mlj3j4=(l1_v4_temp+j3_v4_temp+j4_v4_temp).M()
      if n_tight_jet_in24==3:
        j1_v4_temp.SetPtEtaPhiM(j1_pt,j1_eta,j1_phi,j1_mass)
        j2_v4_temp.SetPtEtaPhiM(j2_pt,j2_eta,j2_phi,j2_mass)
        j3_v4_temp.SetPtEtaPhiM(j3_pt,j3_eta,j3_phi,j3_mass)
    ...
    self.out.fillBranch("cgbH_nl", cgbH_nl)
    self.out.fillBranch("cgbH_region", cgbH_region)
    self.out.fillBranch("cgbH_l1_id", cgbH_l1_id)
    self.out.fillBranch("cgbH_l1_pdgid", cgbH_l1_pdgid)
    self.out.fillBranch("cgbH_l1_pt", cgbH_l1_pt)
    self.out.fillBranch("cgbH_l1_eta", cgbH_l1_eta)
    self.out.fillBranch("cgbH_l1_phi", cgbH_l1_phi)
    self.out.fillBranch("cgbH_l1_mass", cgbH_l1_mass)
    self.out.fillBranch("cgbH_met", cgbH_met)
    self.out.fillBranch("cgbH_met_phi", cgbH_met_phi)
    self.out.fillBranch("cgbH_dr_l1j1", cgbH_dr_l1j1)
    self.out.fillBranch("cgbH_dr_l1j2", cgbH_dr_l1j2)
    self.out.fillBranch("cgbH_dr_l1j3", cgbH_dr_l1j3)
    self.out.fillBranch("cgbH_dr_l1j4", cgbH_dr_l1j4)
    self.out.fillBranch("cgbH_mlj1", cgbH_mlj1)
    self.out.fillBranch("cgbH_mlj2", cgbH_mlj2)
    self.out.fillBranch("cgbH_mlj3", cgbH_mlj3)
    self.out.fillBranch("cgbH_mlj4", cgbH_mlj4)
    self.out.fillBranch("cgbH_mlj1j2", cgbH_mlj1j2)
    self.out.fillBranch("cgbH_mlj1j3", cgbH_mlj1j3)
    self.out.fillBranch("cgbH_mlj1j4", cgbH_mlj1j4)
    self.out.fillBranch("cgbH_mlj2j3", cgbH_mlj2j3)
    self.out.fillBranch("cgbH_mlj2j4", cgbH_mlj2j4)
    self.out.fillBranch("cgbH_mlj3j4", cgbH_mlj3j4)


    if len(tightLeptons)<2:return False

    #  BH retMuons)ion (the requirement on bjet will separate this region to signal region and w-region)
...
#      if (n_tight_muon==1 and tightMuons[0]>muon_pt) or (n_tight_ele==1 and tightElectrons[0]>ele_pt):
    ...
    if bh_nl:
      ...
      if len(tightElectrons)==0 and abs(tightMuons_pdgid[0]+tightMuons_pdgid[1])==0: #and len(tightMuons)==2:#and abs(tightMuons_pdgid[0]+tightMuons_pdgid[1])==26 
        bh_region=1
        bh_l1_id=tightMuons_id[0]
        bh_l2_id=tightMuons_id[1]
        bh_l1_pdgid=tightMuons_pdgid[0]
        bh_l2_pdgid=tightMuons_pdgid[1]
        bh_l1_pt=tightMuons[0].Pt()
        bh_l1_eta=tightMuons[0].Eta()
        bh_l1_phi=tightMuons[0].Phi()
        bh_l1_mass=tightMuons[0].M()
        bh_l2_pt=tightMuons[1].Pt()
        bh_l2_eta=tightMuons[1].Eta()
        bh_l2_phi=tightMuons[1].Phi()
        bh_l2_mass=tightMuons[1].M()
        bh_mll=(tightMuons[0]+tightMuons[1]).M()
        bh_drll=tightMuons[0].DeltaR(tightMuons[1])
        bh_dphill=tightMuons[0].DeltaPhi(tightMuons[1])
      if len(tightElectrons)==1 and abs(tightMuons_pdgid[0]+tightElectrons_pdgid[0])==2:#and len(tightMuons)==1:
        bh_region=2
        bh_l1_id=tightMuons_id[0]
        bh_l1_pdgid=tightMuons_pdgid[0]
        bh_l2_id=tightElectrons_id[0]
        bh_l2_pdgid=tightElectrons_pdgid[0]
        bh_l1_pt=tightMuons[0].Pt()
        bh_l1_eta=tightMuons[0].Eta()
        bh_l1_phi=tightMuons[0].Phi()
        bh_l1_mass=tightMuons[0].M()
        bh_l2_pt=tightElectrons[0].Pt()
        bh_l2_eta=tightElectrons[0].Eta()
        bh_l2_phi=tightElectrons[0].Phi()
        bh_l2_mass=tightElectrons[0].M()
        bh_mll=(tightMuons[0]+tightElectrons[0]).M()
        bh_drll=tightMuons[0].DeltaR(tightElectrons[0])
        bh_dphill=tightMuons[0].DeltaPhi(tightElectrons[0])
      if len(tightElectrons)==2 and abs(tightElectrons_pdgid[0]+tightElectrons_pdgid[1])==0:#and len(tightMuons)==0:
        bh_region=3
        bh_l1_id=tightElectrons_id[0]
        bh_l2_id=tightElectrons_id[1]
        bh_l1_pdgid=tightElectrons_pdgid[0]
        bh_l2_pdgid=tightElectrons_pdgid[1]
        bh_l1_pt=tightElectrons[0].Pt()
        bh_l1_eta=tightElectrons[0].Eta()
        bh_l1_phi=tightElectrons[0].Phi()
        bh_l1_mass=tightElectrons[0].M()
        bh_l2_pt=tightElectrons[1].Pt()
        bh_l2_eta=tightElectrons[1].Eta()
        bh_l2_phi=tightElectrons[1].Phi()
        bh_l2_mass=tightElectrons[1].M()
        bh_mll=(tightElectrons[0]+tightElectrons[1]).M()
        bh_drll=tightElectrons[0].DeltaR(tightElectrons[1])
        bh_dphill=tightElectrons[0].DeltaPhi(tightElectrons[1])
    ...
    if bh_region>0:
      ...
      l2_v4_temp.SetPtEtaPhiM(bh_l2_pt,bh_l2_eta,bh_l2_phi,bh_l2_mass)
      j1_v4_temp=TLorentzVector()
      j2_v4_temp=TLorentzVector()
      j3_v4_temp=TLorentzVector()
      j4_v4_temp=TLorentzVector()
      ...
      if n_tight_jet_in24>3:
        ...
        bh_dr_l2j1=l2_v4_temp.DeltaR(j1_v4_temp)
        bh_dr_l2j2=l2_v4_temp.DeltaR(j2_v4_temp)
        bh_dr_l2j3=l2_v4_temp.DeltaR(j3_v4_temp)
        bh_dr_l2j4=l2_v4_temp.DeltaR(j4_v4_temp)
        bh_mlj1=(l1_v4_temp+l2_v4_temp+j1_v4_temp).M()
        bh_mlj2=(l1_v4_temp+l2_v4_temp+j2_v4_temp).M()
        bh_mlj3=(l1_v4_temp+l2_v4_temp+j3_v4_temp).M()
        bh_mlj4=(l1_v4_temp+l2_v4_temp+j4_v4_temp).M()
        bh_mlj1j2=(l1_v4_temp+l2_v4_temp+j1_v4_temp+j2_v4_temp).M()
        bh_mlj1j3=(l1_v4_temp+l2_v4_temp+j1_v4_temp+j3_v4_temp).M()
        bh_mlj1j4=(l1_v4_temp+l2_v4_temp+j1_v4_temp+j4_v4_temp).M()
        bh_mlj2j3=(l1_v4_temp+l2_v4_temp+j2_v4_temp+j3_v4_temp).M()
        bh_mlj2j4=(l1_v4_temp+l2_v4_temp+j2_v4_temp+j4_v4_temp).M()
        bh_mlj3j4=(l1_v4_temp+l2_v4_temp+j3_v4_temp+j4_v4_temp).M()
      if n_tight_jet_in24==3:
        j1_v4_temp.SetPtEtaPhiM(j1_pt,j1_eta,j1_phi,j1_mass)
        j2_v4_temp.SetPtEtaPhiM(j2_pt,j2_eta,j2_phi,j2_mass)
        j3_v4_temp.SetPtEtaPhiM(j3_pt,j3_eta,j3_phi,j3_mass)
      ...
      if n_bjet_DeepB>2:
        ...
        bh_mlb1=(l1_v4_temp+l2_v4_temp+b1_v4_temp).M()
        bh_mlb2=(l1_v4_temp+l2_v4_temp+b2_v4_temp).M()
        bh_mlb3=(l1_v4_temp+l2_v4_temp+b3_v4_temp).M()
        bh_mlb1b2=(l1_v4_temp+l2_v4_temp+b1_v4_temp+b2_v4_temp).M()
        bh_mlb1b3=(l1_v4_temp+l2_v4_temp+b1_v4_temp+b3_v4_temp).M()
        bh_mlb2b3=(l1_v4_temp+l2_v4_temp+b2_v4_temp+b3_v4_temp).M()
      if n_bjet_DeepB==2:
        b1_v4_temp.SetPtEtaPhiM(DeepB_j1_pt, DeepB_j1_eta,DeepB_j1_phi,DeepB_j1_mass)
        b2_v4_temp.SetPtEtaPhiM(DeepB_j2_pt, DeepB_j2_eta,DeepB_j2_phi,DeepB_j2_mass)
    ...
    self.out.fillBranch("bh_l2_id", bh_l2_id)
    self.out.fillBranch("bh_l1_pdgid", bh_l1_pdgid)
    self.out.fillBranch("bh_l2_pdgid", bh_l2_pdgid)
    ...
    self.out.fillBranch("bh_l2_pt", bh_l2_pt)
    self.out.fillBranch("bh_l2_eta", bh_l2_eta)
    self.out.fillBranch("bh_l2_phi", bh_l2_phi)
    self.out.fillBranch("bh_l2_mass", bh_l2_mass)
    ...
    self.out.fillBranch("bh_dr_l2j1", bh_dr_l2j1)
    self.out.fillBranch("bh_dr_l2j2", bh_dr_l2j2)
    self.out.fillBranch("bh_dr_l2j3", bh_dr_l2j3)
    self.out.fillBranch("bh_dr_l2j4", bh_dr_l2j4)
    ...
    if not (bh_nl or DY_region>0 or cgbH_nl >0):
      return False
